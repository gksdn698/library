$(function(){
	...
	// 첨부파일 목록 불러오기 
	$.getJSON(`${ctxPath}/book/getAttachList`,{bno:bnoValue},function(attachList){
		let fileList = '';
	...
	// 기존 업로드된 파일 삭제
	$('.uploadResultDiv ul').on('click','.toBeDelFile',function(e){
		let listTag =  $(this).closest('li');
		let uuid = listTag.data('uuid');
		if($(this).is(':checked')){ // 체크했을때
			$.ajax({
				type : 'get',
				url  : `${ctxPath}/book/getAttachFileInfo`,
				data : {uuid : uuid},
				success  : function(attach){
					toBeDelList.push(attach);
				}
			})
		} else{ // 체크 해제
	...
	$('.modifyForm button').click(function(){
		...
		if(operation=='remove'){
			formObj.attr('action',`${ctxPath}/book/remove`);
		} else if (operation=='list'){
			formObj.empty();
			addCriteria();
			formObj.attr('action',`${ctxPath}/book/list`)
				   .attr('method','get');
		} else{
			let idx = 0; 
