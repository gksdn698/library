...
@Component
public class BookFileCheckTask {
...
//	@Scheduled(cron = "2 * * * * *")
	public void checkFile() {
		...
		List<Path> fileListPath = fileList.stream()
			.map(vo-> Paths.get("c:/storage/book",vo.getUploadPath(),vo.getUuid()+"_"+vo.getFileName()))
			.collect(Collectors.toList());
		
		// 썸네일 파일 경로 
		fileList.stream()
			.map(vo-> Paths.get("c:/storage/book",vo.getUploadPath(),"s_"+vo.getUuid()+"_"+vo.getFileName()))
			.forEach(e-> fileListPath.add(e));
		// 실제 업로드된 파일 
		File targetDir = Paths.get("c:/storage/book",getYesterdayFolder()).toFile();
